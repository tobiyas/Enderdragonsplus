...
public class DragonDamageWhisperer implements Runnable{
	...
	public void run() {
		...
		if(player == null)
			return;
		double newHealth = dragon.getHealth();
	...
	private void broadcastDamage(Player player, double damage){
		double actualLife = dragon.getHealth();
		double maxLife = dragon.getMaxHealth();
	...
	private String parsePersentageLife(double actual, double max){
		double currentPercentage = actual / max;
